...
void pkgDepCache::MarkKeep0(const PkgIterator &Pkg,bool const Soft,const DbgLogger &DBG)
{
   ...
   {
      DBG.traceTraversal(0, "shouldn't keep a pkg that needs unpack, but is not downloadable");
      return;
   }
   ...
   if (P.Mode == ModeKeep)
   {
      DBG.traceTraversal(0, "already kept");
      return;
   }
   ...
   if (Pkg->VersionList == 0)
   {
      DBG.traceTraversal(0, "can't keep a virtual pkg");
      return;
   }
	       ...
	       if (PkgState[TrgPkg->ID].CandidateVer == *Cur)
               {
                  if (CanSelect++ == 0)
                  {
                     DBG.traceTraversal(2, "found a providing target:", TrgVer);
                     InstVer = TrgVer;
                  }
                  else
                  {
                     DBG.traceTraversal(2, "found another providing target:", TrgVer);
                     break;
                  }
               }
	    }
